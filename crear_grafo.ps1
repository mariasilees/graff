...
<body>
    <div class="container">
        ...
        <div class="warning-box" id="warningBox">
            <h3 id="warningTitle">⚠️ RIESGO DE PRIVACIDAD</h3>
            <span id="warningText">
                ❌ Todos los nodos son identificables<br>
                ❌ Nombres, edades y profesiones visibles<br>
                ❌ Relaciones personales expuestas<br>
                ❌ Grados únicos permiten identificación<br><br>
        ...
        <div class="controls">
            ...
            <button onclick="pauseSimulation()">⏸️ Pausar/Reanudar</button>
        </div>
        <svg id="graph" width="1400" height="800"></svg>
        <div class="tooltip" id="tooltip"></div>
    </div>
    <script>
const originalNodes=[{id:'Ana',edad:22,ciudad:'Barcelona',profesion:'Estudiante'},{id:'Luis',edad:20,ciudad:'Madrid',profesion:'Estudiante'},{id:'María',edad:23,ciudad:'Barcelona',profesion:'Becaria'},{id:'Carlos',edad:21,ciudad:'Valencia',profesion:'Estudiante'},{id:'Sofia',edad:24,ciudad:'Madrid',profesion:'Diseñadora'},{id:'Diego',edad:19,ciudad:'Sevilla',profesion:'Estudiante'},{id:'Laura',edad:22,ciudad:'Barcelona',profesion:'Becaria'},{id:'Pedro',edad:25,ciudad:'Madrid',profesion:'Programador'},{id:'Elena',edad:20,ciudad:'Valencia',profesion:'Estudiante'},{id:'Javier',edad:23,ciudad:'Barcelona',profesion:'Ingeniero'},{id:'Roberto',edad:35,ciudad:'Madrid',profesion:'Arquitecto'},{id:'Carmen',edad:38,ciudad:'Barcelona',profesion:'Abogada'},{id:'Fernando',edad:42,ciudad:'Valencia',profesion:'Médico'},{id:'Isabel',edad:33,ciudad:'Sevilla',profesion:'Profesora'},{id:'Miguel',edad:40,ciudad:'Madrid',profesion:'Empresario'},{id:'Patricia',edad:36,ciudad:'Barcelona',profesion:'Enfermera'},{id:'Andrés',edad:44,ciudad:'Valencia',profesion:'Ingeniero'},{id:'Beatriz',edad:31,ciudad:'Madrid',profesion:'Periodista'},{id:'Jorge',edad:39,ciudad:'Barcelona',profesion:'Contador'},{id:'Rosa',edad:37,ciudad:'Sevilla',profesion:'Farmacéutica'},{id:'Antonio',edad:65,ciudad:'Madrid',profesion:'Jubilado'},{id:'Dolores',edad:68,ciudad:'Barcelona',profesion:'Jubilada'},{id:'Francisco',edad:62,ciudad:'Valencia',profesion:'Jubilado'},{id:'Teresa',edad:70,ciudad:'Sevilla',profesion:'Jubilada'},{id:'José',edad:66,ciudad:'Madrid',profesion:'Jubilado'},{id:'Pilar',edad:64,ciudad:'Barcelona',profesion:'Jubilada'},{id:'Manuel',edad:69,ciudad:'Valencia',profesion:'Jubilado'},{id:'Concepción',edad:63,ciudad:'Madrid',profesion:'Jubilada'},{id:'Ramón',edad:67,ciudad:'Barcelona',profesion:'Jubilado'},{id:'Mercedes',edad:71,ciudad:'Sevilla',profesion:'Jubilada'}];
const originalLinks=[{source:'Ana',target:'Luis'},{source:'Ana',target:'María'},{source:'Ana',target:'Roberto'},{source:'Ana',target:'Carmen'},{source:'Ana',target:'Dolores'},{source:'Ana',target:'Patricia'},{source:'Ana',target:'Javier'},{source:'Ana',target:'Jorge'},{source:'Luis',target:'María'},{source:'Luis',target:'Carmen'},{source:'Luis',target:'Roberto'},{source:'Luis',target:'Antonio'},{source:'Luis',target:'José'},{source:'Luis',target:'Concepción'},{source:'Luis',target:'Miguel'},{source:'María',target:'Patricia'},{source:'María',target:'Dolores'},{source:'María',target:'Pilar'},{source:'María',target:'Ramón'},{source:'María',target:'Jorge'},{source:'Carlos',target:'Sofia'},{source:'Carlos',target:'Fernando'},{source:'Carlos',target:'Elena'},{source:'Carlos',target:'Manuel'},{source:'Carlos',target:'Francisco'},{source:'Sofia',target:'Miguel'},{source:'Sofia',target:'Beatriz'},{source:'Sofia',target:'Antonio'},{source:'Diego',target:'Laura'},{source:'Diego',target:'Isabel'},{source:'Diego',target:'Teresa'},{source:'Laura',target:'Patricia'},{source:'Pedro',target:'Beatriz'},{source:'Pedro',target:'Antonio'},{source:'Elena',target:'Jorge'},{source:'Roberto',target:'Carmen'},{source:'Fernando',target:'Miguel'},{source:'Patricia',target:'Isabel'},{source:'Andrés',target:'Rosa'},{source:'Dolores',target:'Pilar'},{source:'Francisco',target:'Manuel'},{source:'Teresa',target:'Mercedes'},{source:'Ramón',target:'Concepción'}];
...
function detectCommunities(){
    ...
    if(largestSize>6){
        console.log('🔍 Subdividiendo comunidad grande ('+largestSize+' nodos)...');
        ...
        console.log('  ✓ Subdividida en '+Object.keys(finalSubComms).length+' subcomunidades:');
    ...
    console.log('✓ Detectadas '+communities.length+' comunidades');
    communities.forEach((comm,idx)=>{
        console.log('  Comunidad '+(idx+1)+': '+comm.size+' nodos, '+comm.internalEdges+' aristas internas, '+comm.externalEdges+' aristas externas');
    });
    let statsHTML='<strong>ESTADÍSTICAS POR COMUNIDAD:</strong><br><br>';
    communities.forEach((comm,idx)=>{
        statsHTML+='<strong style="color:'+comm.color+'">📊 Comunidad '+(idx+1)+'</strong><br>';
        statsHTML+='   • Tamaño: '+comm.size+' personas<br>';
        statsHTML+='   • Edad media: '+comm.avgAge+' años<br>';
        statsHTML+='   • Ciudades: '+Object.keys(comm.cities).map(c=>c+' ('+comm.cities[c]+')').join(', ')+'<br>';
        statsHTML+='   • Conexiones internas: '+comm.internalEdges+'<br>';
        statsHTML+='   • Conexiones externas: '+comm.externalEdges+'<br><br>';
    });
    warningBox.classed('protected',true).classed('anonymous',false).classed('kanon',false).classed('automorph',false);
    warningTitle.html('🔍 COMUNIDADES DETECTADAS (Algoritmo Louvain)');
    ...
    setTimeout(()=>{
        ...
        supernodes.forEach((sn,idx)=>{
            generalHTML+='<strong style="color:'+sn.color+'">🔒 '+sn.id+'</strong><br>';
            generalHTML+='   • Personas: '+sn.count+'<br>';
            generalHTML+='   • Edad media: '+sn.avgAge+' años<br>';
            generalHTML+='   • Ciudad principal: '+Object.keys(sn.cityCounts).sort((a,b)=>sn.cityCounts[b]-sn.cityCounts[a])[0]+'<br>';
            generalHTML+='   • Profesión principal: '+Object.keys(sn.profCounts).sort((a,b)=>sn.profCounts[b]-sn.profCounts[a])[0]+'<br><br>';
        });
        warningBox.classed('protected',false).classed('anonymous',true).classed('kanon',false).classed('automorph',false);
        warningTitle.html('🔒 GENERALIZACIÓN DE COMUNIDADES');
        warningText.html('🔒 Identidades individuales eliminadas<br>🔒 Solo promedios y agregados<br>🔒 '+supernodes.length+' supernodos comunitarios<br>🔒 '+intercommunityLinks.length+' conexiones entre comunidades<br><br>'+generalHTML+'<strong style="color:#27ae60">✓ Máxima privacidad garantizada</strong>');
        updateGraph();
    },4000);
}
function applyEdgePerturbation(){
    ...
    console.log('🔀 APLICANDO PERTURBACIÓN CON DIFFERENTIAL PRIVACY');
    console.log('🔀 Epsilon: '+epsilon);
    console.log('🔀 Probabilidad mantener arista existente: '+p_real.toFixed(4));
    console.log('🔀 Probabilidad añadir arista inexistente: '+p_false.toFixed(4));
    ...
    console.log('✓ Aristas añadidas: '+edgesAdded);
    console.log('✓ Aristas eliminadas: '+edgesRemoved);
    console.log('✓ Total aristas en grafo protegido: '+links.length);
    warningBox.classed('protected',false).classed('anonymous',false).classed('kanon',false).classed('automorph',true);
    warningTitle.html('🔀 DIFFERENTIAL PRIVACY APLICADA (ε='+epsilon+')');
    warningText.html('🔀 <strong>RANDOMIZED RESPONSE MECHANISM</strong><br><br>🔀 Epsilon (ε): '+epsilon+'<br>🔀 P(mantener arista): '+p_real.toFixed(3)+'<br>🔀 P(añadir arista): '+p_false.toFixed(3)+'<br><br>📊 <strong>Resultado:</strong><br>🔀 '+edgesAdded+' aristas añadidas<br>🔀 '+edgesRemoved+' aristas eliminadas<br>🔀 Total: '+links.length+' aristas<br><br><strong style="color:#27ae60">✓ Privacidad diferencial garantizada</strong>');
...
function applyKAnonymity(){
    ...
    warningTitle.html('🟣 K-ANONIMIDAD APLICADA (k=3)');
    let statsText='';
    const allSatisfy=degreeGroups.every(deg=>finalDegreeMap[deg].length>=k);
    degreeGroups.forEach(deg=>{const count=finalDegreeMap[deg].length;const status=count>=k?'✓':'⚠';statsText+=status+' Grado '+deg+': '+count+' nodos<br>';});
    warningText.html('🟣 Aristas añadidas (líneas moradas)<br>🟣 Cada grado compartido por ≥3 nodos<br>🟣 '+newLinks.length+' conexiones añadidas<br><br><strong>Distribución final:</strong><br>'+statsText+(allSatisfy?'<br><strong style="color:#27ae60">✓ K-anonimidad satisfecha</strong>':'<br><strong style="color:#e74c3c">⚠ Puede necesitar más aristas</strong>'));
...
function toggleClusters(){
    ...
    if(clustered){
        ...
        warningTitle.html('🏙️ PROTECCIÓN MEJORADA');
        ...
        warningTitle.html('⚠️ RIESGO DE PRIVACIDAD');
...
function fusionNodes(){
    ...
    warningTitle.html('🔒 MÁXIMA PROTECCIÓN');
    warningText.html('🔒 Solo ciudades visibles<br>🔒 '+links.length+' conexiones entre ciudades<br>🔒 Identidades completamente anonimizadas<br>🔒 Solo datos agregados disponibles<br><br><strong>4 Supernodos<br>'+originalNodes.length+' personas protegidas</strong>');
    clusterBtn.html('🏙️ Supernodos Activos');
    updateGraph();
}
...
Write-Host "✅ Archivo 'grafo_interactivo.html' creado correctamente!" -ForegroundColor Green
Write-Host "📂 Ubicación: $(Get-Location)\grafo_interactivo.html" -ForegroundColor Cyan